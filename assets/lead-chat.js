    ...
    const submitLead = async () => {
      try {
        const payload = {
          ...
          lead_id: (window.crypto && crypto.randomUUID) ? crypto.randomUUID() : ("lead-" + Date.now()),
          source_domain: window.location.hostname,
          source_page: window.location.pathname,
          form_name: "lead-chat",
          lead_type: "chat",
          message: String(state.notes || state.project_type || ""),
          form_started_at: startedAt,
          form_js: formProof(startedAt),
          form_pow: solvePow(startedAt),
          form_int: String(Math.max(formInt, 2)),
          "bot-field": "",
          website: ""
        };
        ...
        var ingestRes = await fetch("/.netlify/functions/lead-ingest", {
          method: "POST",
